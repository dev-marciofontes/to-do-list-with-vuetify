<template>
  <div class="ma-4 text-center">
    ...
    <p class="ma-4">Abaixo, listo algumas tecnologias que usei neste projeto.</p>
    <v-divider></v-divider>
  </div>
  <div></div>
  <div class="ma-4 d-flex">
    <AboutCard v-for="(item, index) in items" :key="index" :image="item.image" :title="item.title"
      :subtitle="item.subtitle" :text="item.text" />
  </div>

  <div class="text-center">
    <v-divider class="ma-4"></v-divider>
    <a target="_blank" href="https://link-service.netlify.app/" alt="Minhas mídias sociais">
